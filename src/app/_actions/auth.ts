...
export async function setAuthCookiesAndRedirect(
  ...
  cookieStore.set("sonata_access_token", accessToken, {
    httpOnly: true,
  ...
  cookieStore.set("sonata_refresh_token", refreshToken, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
 ...
 * 
 * @param redirectPath - The path to redirect to after clearing cookies (defaults to /signin/teacher)
 */
export async function clearAuthAndRedirect(redirectPath: string = "/signin/teacher") {
  const cookieStore = await cookies();

  // Clear access token cookie
  cookieStore.delete("sonata_access_token");

  // Clear refresh token cookie
  cookieStore.delete("sonata_refresh_token");
